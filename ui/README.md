...
This particular template is designed to use `cargo-leptos` with client-side rendering. This allows you to use `cargo-leptos` features like code splitting/lazy loading with CSR.

The server is a simple static file server. The code is designed such that `cargo leptos serve` will write a static `index.html` file into the `target/site` directory, which means that `target/site` can be deployed to any static file server.
...
Then run
```bash
cargo leptos new --git https://github.com/leptos-rs/start-csr/
```
