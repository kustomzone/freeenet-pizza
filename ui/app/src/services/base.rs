...
impl BaseInterface for LocalStorageService {
    fn get_contracts(&self) -> Result<Vec<String>, Box<dyn Error>> {
        match self.storage.get_item(CONTRACTS_KEY)? {
    ...
    fn get_contract_parameters_and_state(
        ...
        let state_json = self.storage.get_item(&Self::get_state_key(&id))?
            .ok_or(format!("state not found for {}", id))?;
        let params_json = self.storage.get_item(&Self::get_params_key(&id))?
    ...
    fn publish_delta(&self, id: String, delta: FullOrderStateV1Delta
        ...
        let (mut state, params) = self.get_contract_parameters_and_state(id.clone())?;
        state.apply_delta(&state, &params, &Some(delta)).map_err(|e| e.to_string())?;

        let state_json = serde_json::to_string(&state)?;
        self.storage.set_item(&Self::get_state_key(&id), &state_json)?;
    ...
    fn get_private_key(&self) -> Result<SigningKey, Box<dyn Error>> {
        match self.storage.get_item(PRIVATE_KEY_KEY)? {
            Some(s) => {
            ...
            None => {
                ...
                self.storage.set_item(PRIVATE_KEY_KEY, &hex_key)?;
                Ok(signing_key)
            }
        }
    }
