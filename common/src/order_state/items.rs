...
/*impl ItemsV1 {
    ...
    pub fn rebuild_actions_state_with_decrypted(
        &mut self,
        decrypted_content: &HashMap<ItemId, Vec<u8>>,
    ) {
        ...
        let message_authors: HashMap<ItemId, MemberId> = self
            .items
            .iter()
            .filter(|m| !m.item.content.is_action())
            .map(|m| (m.id(), m.item.author))
            .collect();
    ...
    pub fn effective_text(&self, message: &AuthorizedItemV1) -> Option<String> {
        let id = message.id();
        ...
        message.item.content.as_public_string()
    }

    /// Get reactions for a message
    pub fn reactions(&self, message_id: &ItemId) -> Option<&HashMap<String, Vec<MemberId>>> {
        self.actions_state.reactions.get(message_id)
    }

    /// Get all non-deleted, non-action messages for display
    pub fn display_messages(&self) -> impl Iterator<Item = &AuthorizedItemV1> {
        self.items.iter().filter(|m| {
...
impl ItemActionBody {
    ...
    pub fn edit(target: ItemId, new_text: String) -> Self {
        use crate::room_state::content::{
    ...
    pub fn delete(target: ItemId) -> Self {
        use crate::room_state::content::{
            ActionContentV1, ACTION_CONTENT_VERSION, CONTENT_TYPE_ACTION,
        };
    ...
    pub fn reaction(target: ItemId, emoji: String) -> Self {
        use crate::room_state::content::{
            ActionContentV1, ACTION_CONTENT_VERSION, CONTENT_TYPE_ACTION,
        };
    ...
    pub fn remove_reaction(target: ItemId, emoji: String) -> Self {
        use crate::room_state::content::{
            ActionContentV1, ACTION_CONTENT_VERSION, CONTENT_TYPE_ACTION,
        };
...
impl fmt::Display for ItemActionBody {
...
mod tests {
    ...
    fn create_test_message(owner_id: MemberId, author_id: MemberId) -> ItemV1 {
        ItemV1 {
            room_owner: owner_id,
            author: author_id,
            time: SystemTime::now(),
            content: ItemActionBody::public("Test message".to_string()),
        }
    }
    ...
    fn test_authorized_message_v1_debug() {
        ...
        let authorized_message = AuthorizedItemV1::new(message, &signing_key);
    ...
    fn test_authorized_message_new_and_validate() {
        ...
        let message = create_test_message(owner_id, author_id);
        let authorized_message = AuthorizedItemV1::new(message.clone(), &signing_key);

        assert_eq!(authorized_message.item, message);
        ...
        tampered_message.item.content = ItemActionBody::public("Tampered content".to_string());
    ...
    fn test_message_id() {
        ...
        let authorized_message2 = AuthorizedItemV1::new(message2, &signing_key);
    ...
    fn test_messages_verify() {
        ...
        let message = create_test_message(owner_id, author_id);
        let authorized_message = AuthorizedItemV1::new(message, &author_signing_key);
        ...
        let invalid_authorized_message =
            AuthorizedItemV1::new(invalid_message, &author_signing_key);
    ...
    fn test_messages_summarize() {
        ...
        let authorized_message1 = AuthorizedItemV1::new(message1, &signing_key);
        let authorized_message2 = AuthorizedItemV1::new(message2, &signing_key);
    ...
    fn test_messages_delta() {
        ...
        let authorized_message1 = AuthorizedItemV1::new(message1, &signing_key);
        let authorized_message2 = AuthorizedItemV1::new(message2, &signing_key);
        let authorized_message3 = AuthorizedItemV1::new(message3, &signing_key);
        ...
        let empty_summary: Vec<ItemId> = vec![];
    ...
    fn test_messages_apply_delta() {
        ...
        let create_message = |time: SystemTime| {
            ...
            AuthorizedItemV1::new(message, &author_signing_key)
        };
    ...
    fn test_message_author_preservation_across_users() {
        ...
        let msg1 = ItemV1 {
            room_owner: owner_id,
            author: user1_id,
            content: ItemActionBody::public("Message from user1".to_string()),
            time: SystemTime::now(),
        };
        ...
        let msg2 = ItemV1 {
            room_owner: owner_id,
            author: user2_id,
            content: ItemActionBody::public("Message from user2".to_string()),
            time: SystemTime::now() + Duration::from_secs(1),
        };

        let auth_msg1 = AuthorizedItemV1::new(msg1.clone(), &user1_sk);
        let auth_msg2 = AuthorizedItemV1::new(msg2.clone(), &user2_sk);
    ...
    fn test_edit_action() {
        ...
        let original_msg = ItemV1 {
            room_owner: owner_id,
            author: author_id,
            time: SystemTime::now(),
            content: ItemActionBody::public("Original content".to_string()),
        };
        let auth_original = AuthorizedItemV1::new(original_msg, &signing_key);
        ...
        let edit_msg = ItemV1 {
            ...
            author: author_id,
            time: SystemTime::now() + Duration::from_secs(1),
            content: ItemActionBody::edit(original_id.clone(), "Edited content".to_string()),
        };
        let auth_edit = AuthorizedItemV1::new(edit_msg, &signing_key);
    ...
    fn test_edit_by_non_author_ignored() {
        ...
        let auth_original = AuthorizedItemV1::new(original_msg, &owner_sk);
        let original_id = auth_original.id();
        ...
        let edit_msg = ItemV1 {
            ...
            author: other_id,
            time: SystemTime::now() + Duration::from_secs(1),
            content: ItemActionBody::edit(original_id.clone(), "Hacked content".to_string()),
        };
        let auth_edit = AuthorizedItemV1::new(edit_msg, &other_sk);
    ...
    fn test_delete_action() {
        ...
        let original_msg = ItemV1 {
            ...
            author: owner_id,
            time: SystemTime::now(),
            content: ItemActionBody::public("Will be deleted".to_string()),
        };
        let auth_original = AuthorizedItemV1::new(original_msg, &signing_key);
        let original_id = auth_original.id();

        // Create delete action
        let delete_msg = ItemV1 {
            room_owner: owner_id,
            author: owner_id,
            time: SystemTime::now() + Duration::from_secs(1),
            content: ItemActionBody::delete(original_id.clone()),
        };
        let auth_delete = AuthorizedItemV1::new(delete_msg, &signing_key);
    ...
    fn test_reaction_action() {
        ...
        let original_msg = ItemV1 {
            ...
            author: user1_id,
            time: SystemTime::now(),
            content: ItemActionBody::public("React to me!".to_string()),
        };
        let auth_original = AuthorizedItemV1::new(original_msg, &user1_sk);
        let original_id = auth_original.id();

        // Create reaction from user2
        let reaction_msg = ItemV1 {
            room_owner: owner_id,
            author: user2_id,
            time: SystemTime::now() + Duration::from_secs(1),
            content: ItemActionBody::reaction(original_id.clone(), "👍".to_string()),
        };
        let auth_reaction = AuthorizedItemV1::new(reaction_msg, &user2_sk);

        // Create another reaction from user1
        let reaction_msg2 = ItemV1 {
            room_owner: owner_id,
            author: user1_id,
            time: SystemTime::now() + Duration::from_secs(2),
            content: ItemActionBody::reaction(original_id.clone(), "👍".to_string()),
        };
        let auth_reaction2 = AuthorizedItemV1::new(reaction_msg2, &user1_sk);
    ...
    fn test_remove_reaction_action() {
        ...
        let auth_original = AuthorizedItemV1::new(original_msg, &user_sk);
        let original_id = auth_original.id();
        ...
        let reaction_msg = ItemV1 {
            ...
            author: user_id,
            time: SystemTime::now() + Duration::from_secs(1),
            content: ItemActionBody::reaction(original_id.clone(), "❤️".to_string()),
        };
        let auth_reaction = AuthorizedItemV1::new(reaction_msg, &user_sk);

        // Remove reaction
        let remove_msg = ItemV1 {
            room_owner: owner_id,
            author: user_id,
            time: SystemTime::now() + Duration::from_secs(2),
            content: ItemActionBody::remove_reaction(original_id.clone(), "❤️".to_string()),
        };
        let auth_remove = AuthorizedItemV1::new(remove_msg, &user_sk);
    ...
    fn test_action_on_deleted_message_ignored() {
        ...
        let edit_msg = ItemV1 {
            ...
            author: owner_id,
            time: SystemTime::now() + Duration::from_secs(2),
            content: ItemActionBody::edit(original_id.clone(), "Too late!".to_string()),
        };
        let auth_edit = AuthorizedItemV1::new(edit_msg, &signing_key);
    ...
    fn test_display_messages_filters_actions() {
        ...
        let msg1 = ItemV1 {
            ...
            author: owner_id,
            time: SystemTime::now(),
            content: ItemActionBody::public("Hello".to_string()),
        };
        let auth_msg1 = AuthorizedItemV1::new(msg1, &signing_key);
        ...
        let reaction_msg = ItemV1 {
            ...
            author: owner_id,
            time: SystemTime::now() + Duration::from_secs(1),
            content: ItemActionBody::reaction(msg1_id, "👍".to_string()),
        };
        let auth_reaction = AuthorizedItemV1::new(reaction_msg, &signing_key);
        ...
        let msg2 = ItemV1 {
            ...
            author: owner_id,
            time: SystemTime::now() + Duration::from_secs(2),
            content: ItemActionBody::public("World".to_string()),
        };
        let auth_msg2 = AuthorizedItemV1::new(msg2, &signing_key);
